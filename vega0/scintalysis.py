...
def get_yamls():
    ...
    return yaml_data[22872:22942] + yaml_data[22943:23013]
...
def scalevstime_plots(night1_obsids, night2_obsids, source, plotname, night1_mdnmetric, night2_mdnmetric):
    ...
    ax1.plot_date(mwadates1, night1_ampscales, 'o', linestyle = ':', color='g')
    ax1.errorbar(mwadates1, night1_ampscales, yerr=night1_stds, linestyle='None')
    text1 = ax1.annotate('rms=%s'%(str(rms1)), xy=(mwadates1[0], 1.35),color='m')
    text1.set_fontsize(20)
    ax1.set_ylabel('Median amplitude scaling factor')
    ax1.set_xlabel('Obsid date, time in 10/2013')
    ax1.set_title('%s, %s Jy, mean qam: %s'%(source,str(round(np.nanmean(night1_flux),2)), str(round(night1_mdnmetric,2))))
    ax1.set_ylim([0.6, 1.4])
    ax1.grid()

    ax2 = fig.add_subplot(122)
    ax2.plot_date(mwadates2, night2_ampscales, 'o', linestyle = ':', color='g')
    ax2.errorbar(mwadates2, night2_ampscales, yerr=night2_stds,linestyle='None')
    text2 = ax2.annotate('rms=%s'%(str(rms2)), xy=(mwadates2[0], 1.35),color='m')
    text2.set_fontsize(20)
    ax2.set_ylabel('Median amplitude scaling factor')
    ax2.set_xlabel('Obsid date, time in 10/2013')
...
if __name__ == '__main__':
    ...
    for i in range(len(trackable_sources)):
        print('.......................................................This is source %s'%(str(i+1)))
        #trackable_sources = sources_to_track(obsidsets)
        #print(trackable_sources[:11])
        source_to_track = trackable_sources[i]
        scalevstime_plots(night1_obsids, night2_obsids, source_to_track, '%s.png'%(source_to_track), night1_mdnmetric, night2_mdnmetric)
